...
import { setupMigratedToken, log, signAndSend, bpsToPercent } from './setup'
...
const main = async () => {
  ...
  console.log('BOT MODULE TEST — Surfpool Mainnet Fork')
  console.log('='.repeat(60))
  ...
  try {
    tokens = await scanForLendingMarkets(connection, tokens, 20, testLogger.child('scanner'))

    const found = tokens.get(mint)
    if (found) {
      ok(
        'scanner',
        `found ${tokens.size} tokens, our token has ${found.lendingInfo.active_loans} active loans`,
      )
    } else {
      // on Surfpool fork, getTokens doesn't index local tokens.
      // build the MonitoredToken manually so we can test the other modules.
      log('  note: getTokens does not index local fork tokens — building MonitoredToken directly')

      const tokenDetail = await getToken(connection, mint)
      ...
      ok(
        'scanner fallback',
        `built MonitoredToken for ${tokenDetail.symbol}, loans=${lending.active_loans}`,
      )
    }
  } catch (e: any) {
    fail('scanner', e)
  ...
  try {
    const profile = await profiler.profile(connection, borrowerAddr, mint)
    ...
    const cached = await profiler.profile(connection, borrowerAddr, mint)
  ...
  try {
    ...
    const position = await getLoanPosition(connection, mint, borrowerAddr)
  ...
  try {
    ...
    const config: BotConfig = {
      ...
      vaultCreator: wallet.publicKey.toBase58(),
    ...
    const result = await liquidator.tryLiquidate(connection, scored)
